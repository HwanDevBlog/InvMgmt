...
stateDiagram-v2
    ...
    RESERVED --> EXPIRED: 예약 만료 및 재고 복원
    CONFIRMED --> CANCELED: 반품 전 주문 취소
    CONFIRMED --> CONFIRMED: 부분 반품
    CONFIRMED --> RETURNED: 전량 반품
```

허용되지 않은 상태에서 명령을 실행하면 비즈니스 충돌로 처리합니다. 예약 만료는 `RESERVED` 상태에서만 가능하며 차감했던 재고를 복원하고 원장을 남깁니다. 부분 반품 중에는 `CONFIRMED` 상태를 유지하고 모든 수량이 반품된 시점에 `RETURNED`로 전환합니다. 이미 일부라도 반품된 주문은 취소할 수 없어 재고가 중복 복원되지 않습니다.
...
`demoRun`은 운영 JAR에 포함되지 않는 로컬 전용 데이터 로더로 일반 상품 4개와 서로 다른 처리 상태의 주문 6개를 생성합니다. `localRun`은 기존처럼 빈 데이터베이스로 시작합니다.
...
재고 화면의 `정합성 확인` 버튼을 누르면 현재고와 원장 합계를 대사합니다. 결과에는 조회 시각과 일치·불일치 건수를 표시하고, 불일치 상품은 현재고·원장 합계·차이를 펼쳐 보여줍니다.

주문 화면에서 상품과 수량을 선택해 주문을 생성하고, 예약·확정·예약 만료·취소·부분 반품까지 처리할 수 있습니다. 주문이 변경되면 관련 목록을 갱신하며, 상태 변경 요청은 실패 후 재시도할 때 동일한 멱등키를 사용합니다. 일부라도 반품된 주문은 취소할 수 없습니다.
...
| `POST` | `/api/orders/{orderId}/expire` | 예약 만료와 재고 복원 |
...
- 예약 만료·취소와 부분·전체 반품 수량이 재고와 원장에 함께 반영되는지
...
- 재고 화면에서 대사 요청 전·일치·불일치·조회 실패 상태를 구분하는지
- 주문 화면의 생성·예약·확정·취소·반품 후 목록이 갱신되고, 실패한 처리 요청의 재시도에 같은 멱등키를 사용하는지

GitHub Actions에서도 푸시와 Pull Request마다 PostgreSQL 16 기반 백엔드 테스트와 프론트엔드 타입 검사·테스트·빌드를 실행합니다.

## 개발 방식

이 프로젝트는 구현과 학습 과정에서 OpenAI Codex를 개발 보조 도구로 활용했습니다.

프로젝트의 요구사항과 도메인 범위는 직접 정의했으며, AI가 제안하거나 생성한 코드는 로컬 실행, 자동화 테스트와 CI를 통해 검증했습니다. 주요 설계 판단과 구현 내용은 면접에서 직접 설명할 수 있도록 별도로 정리했습니다.
