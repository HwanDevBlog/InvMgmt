...
주문 화면에서는 `생성` 상태의 주문을 재고 예약하고, `재고 예약` 상태의 주문을 확정할 수 있습니다. 예약에 성공하면 주문·현재고·재고 원장 조회가 갱신되며, 실패한 요청을 재시도할 때는 동일한 멱등키를 사용합니다.
...
- 주문 화면의 예약·확정 후 목록이 갱신되고, 실패한 요청의 재시도에 같은 멱등키를 사용하는지
